...
public:
    FilmGUI(MovieRent& service, ShoppingCart& cos) : service{ service }, cos{ cos } {
        initGUI();
        loadData();
        initConnect();
    }
private:
    ...
    QString numarFilme = QString::number(service.getMovies().size());
    ...
    void initConnect() {
        ...
        QObject::connect(butonAdauga, &QPushButton::clicked, [&]() {
            ...
            try {
                service.addMovie(titluFilm, genFilm, anAparitie, actorPrincipal);
            }
            catch (RepoException& exceptie) {
                QMessageBox::warning(this, "Atenție!", QString::fromStdString(exceptie.getMessage()));
            }
            catch (ValidationError& exceptie) {
                QMessageBox::warning(this, "Atenție!", QString::fromStdString(exceptie.getFullMessage()));
            }
        ...
        QObject::connect(butonModifica, &QPushButton::clicked, [&]() {
            ...
            try {
                service.updateMovie(titluFilm, genFilm, anAparitie, actorPrincipal);
            }
            catch (RepoException& exceptie) {
                QMessageBox::warning(this, "Atenție!", QString::fromStdString(exceptie.getMessage()));
            }
        ...
        QObject::connect(butonCauta, &QPushButton::clicked, [&]() {
            ...
            QObject::connect(butonCautareDialog, &QPushButton::clicked, [=]() {
                if (lineEditTitlu->text() != "") {
                    try {
                        auto film = service.searchMovie(lineEditTitlu->text().toStdString());
                        listaFilme->clear();
        ...
        QObject::connect(butonFiltreaza, &QPushButton::clicked, [&]() {
            ...
            QObject::connect(butonFiltrareDialog, &QPushButton::clicked, [=]() {
                if (lineEditGen->text() != "" && lineEditAn->text() == "") {
                    auto filme = service.filterGenre(lineEditGen->text().toStdString());
                    listaFilme->clear();
                ...
                if (lineEditGen->text() == "" && lineEditAn->text() != "") {
                    auto filme = service.filterYear(lineEditAn->text().toInt());
                    listaFilme->clear();
                    for (const auto& film : filme) {
                        QString item = QString::fromStdString(film.getTitle() + ", " + film.getGenre() + ", " + ", " +
                            std::to_string(film.getYear()) + ", " + film.getMainChar());
        ...
        QObject::connect(butonSorteaza, &QPushButton::clicked, [&]() {
            ...
            QObject::connect(butonSortareDialog, &QPushButton::clicked, [=]() {
                if (lineEditReperSortare->text() != "" && lineEditOrdineSortare->text() != "") {
                    auto filme = service.sorting(static_cast<SortType>(lineEditReperSortare->text().toInt()), static_cast<SortOrder>(lineEditOrdineSortare->text().toInt()));
                    listaFilme->clear();
                    for (const auto& film : filme) {
                        QString item = QString::fromStdString(film.getTitle() + ", " + film.getGenre() + ", " +
                            std::to_string(film.getYear()) + ", " + film.getMainChar());
        ...
        QObject::connect(butonAdaugareInCos, &QPushButton::clicked, [&]() {
            ...
            try {
                auto iterator = service.searchMovie(titluFilm);
            }
            catch (RepoException& exceptie) {
            ...
            for (const auto& film : service.getAllCart()) {
                if (titluFilm == film.getTitle()) {
        ...
        QObject::connect(butonAdaugareRandomInCos, &QPushButton::clicked, [&]() {
            ...
            int numarDeFilmeIntroduse = service.getMovies().size();
            if (numarAdaugareRandomInCosEdit->text().toInt() <= numarDeFilmeIntroduse)
                service.randomCart(numarAdaugareRandomInCosEdit->text().toInt());
            else
        ...
        QObject::connect(butonExportInCSV, &QPushButton::clicked, [&]() {
            ...
            int numarDeFilmeIntroduse = service.getMovies().size();
            try {
                service.exportCSV(numeFisierCSV->text().toStdString(), service.getAllCart());
            }
            catch (RepoException& exceptie) {
                QMessageBox::warning(this, "Atenție!", QString::fromStdString(exceptie.getMessage()));
            }
        ...
        QObject::connect(butonExportInHTML, &QPushButton::clicked, [&]() {
            ...
            int numarDeFilmeIntroduse = service.getMovies().size();
            try {
                service.exportHTML(numeFisierHTML->text().toStdString(), service.getAllCart());
            }
            catch (RepoException& exceptie) {
                QMessageBox::warning(this, "Atenție!", QString::fromStdString(exceptie.getMessage()));
            }
    ...
    void loadData() {
        ...
        auto filme = service.getMovies();
        for (const auto& film : filme) {
            int row = tabelFilme->rowCount();
            tabelFilme->insertRow(row);

            QTableWidgetItem* titluItem = new QTableWidgetItem(QString::fromStdString(film.getTitle()));
            QTableWidgetItem* genItem = new QTableWidgetItem(QString::fromStdString(film.getGenre()));
            QTableWidgetItem* anAparitieItem = new QTableWidgetItem(QString::number(film.getYear()));
            QTableWidgetItem* actorPrincipalItem = new QTableWidgetItem(QString::fromStdString(film.getMainChar()));
        ...
        int numarFilme = service.getMovies().size();
