...
void MovieListWindow::initializeGUI() {
    ...
    openCrudCartButton = new QPushButton("Open Editable Cart", this);
    openReadOnlyCartButton = new QPushButton("Open ReadOnly Cart", this);

    auto* cartButtonsLayout = new QHBoxLayout;
    cartButtonsLayout->addWidget(openCrudCartButton);
    cartButtonsLayout->addWidget(openReadOnlyCartButton);
...
void MovieListWindow::connectSignalsSlots() {
    ...
    QObject::connect(addToCartButton, &QPushButton::clicked, [this]() {
    auto selectedItem = movieList->currentItem();
    if (!selectedItem) {
        QMessageBox::warning(this, "Warning", "Please select a movie to add to the cart.");
        return;
    }
    std::string title = selectedItem->text().toStdString();
    try {
        srv.addToCart(title);  // aici folosești funcția ta
        QMessageBox::information(this, "Success", "Movie added to cart.");
    } catch (const RepoException& e) {
QMessageBox::warning(this, "Repository Error", QString::fromStdString(e.getMessage()));
